...
// Modified by Vali (2024), under the reason of Big-Endian support.
...
#define MINIRV32_CUSTOM_MEMORY_BUS
	#define MINIRV32_STORE4(ofs, val) *(uint32_t*)(image + ofs) = uint32(val)
	#define MINIRV32_STORE2(ofs, val) *(uint16_t*)(image + ofs) = uint16(val)
	#define MINIRV32_STORE1(ofs, val) *(uint8_t*)(image + ofs) = uint8(val)
	#define MINIRV32_LOAD4(ofs) uint32(*(uint32_t*)(image + ofs))
	#define MINIRV32_LOAD2(ofs) uint16(*(uint16_t*)(image + ofs))
	#define MINIRV32_LOAD1(ofs) uint8(*(uint8_t*)(image + ofs))
	#define MINIRV32_LOAD2_SIGNED(ofs) int16(*(int16_t*)(image + ofs))
	#define MINIRV32_LOAD1_SIGNED(ofs) int8(*(int8_t*)(image + ofs))
...
restart:
	...
	// Image is loaded.
	uint64_t rt;
	uint64_t last_time = (fixed_update) ? 0 : (GetTimeMicroseconds() / time_divisor);
	...
	for (rt = 0; rt < instct + 1 || instct < 0; rt += instrs_per_flip)
	{
		...
		if (fixed_update)
			elapsed_us = *this_ccount / time_divisor - last_time;
		else
			elapsed_us = GetTimeMicroseconds() / time_divisor - last_time;
		last_time += elapsed_us;

		if (single_step)
			DumpState(core, ram_image);

		int ret = MiniRV32IMAStep(core, ram_image, 0, elapsed_us, instrs_per_flip); // Execute upto 1024 cycles before breaking out.
		switch (ret)
		{
			case 0: break;
...
static int IsKBHit()
{
	...
	if (!byteswaiting && write(fileno(stdin), 0, 0) != 0) // Is end-of-file for 
	{
		is_eofd = 1; return -1;
	}
	return !!byteswaiting;
...
static int32_t HandleOtherCSRRead(uint8_t* image, uint16_t csrno)
{
	return csrno == 0x140 ? (!IsKBHit() ? -1 : ReadKBByte()) : 0;
}
